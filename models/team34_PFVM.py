...
class ParameterFreeVisionMamba(nn.Module):
    r""" ParameterFreeVisionMamba Model
    ...
    def __init__(self,
                 ...
                 **kwargs):
        super(ParameterFreeVisionMamba, self).__init__()
        num_in_ch = in_chans
        num_out_ch = in_chans
        num_feat = 64
...
class ParameterFreeAttetion(nn.Module):
    def __init__(self, num_feat, is_light_sr=False):
        super(ParameterFreeAttetion, self).__init__()
        if is_light_sr:  # we use dilated-conv & DWConv for lightSR for a large ERF
            self.conv3x3 = nn.Conv2d(in_channels=num_feat, out_channels=num_feat, kernel_size=3, padding=1, stride=1,
                                       bias=True)
            self.act = torch.nn.SiLU(inplace=True)

    def forward(self, x):
        out = self.act(self.conv3x3(x))
